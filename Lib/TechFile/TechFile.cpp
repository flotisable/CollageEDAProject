...
// TechFile public member function
...
double TechFile::rule( SpacingRule::Type type , Layer::Type layer1 ,
                                                Layer::Type layer2 )
{
  for( SpacingRule &rule : rules[type] )
     if( rule.layer1 == layer1 &&
         ( layer2 == Layer::UNKNOWN || rule.layer2 == layer2 ) )
       return rule.value;
  return -1;
}
// end TechFile public member function

// TechFile private member function
...
void TechFile::readSpacingRule()
{
  ...
  catchWord(); spacingRule.layer1 = Layer::map      ( word );
  catchWord(); spacingRule.layer2 = Layer::map      ( word );
  catchWord(); spacingRule.value  = stod            ( word );

  if( Layer::map( spacingRule.layer2 ) == word )
    spacingRule.layer2 = Layer::UNKNOWN;
...
void TechFile::readOrderedSpacingRule()
{
  ...
  catchWord(); orderedSpacingRule.layer1  = Layer::map      ( word );
  catchWord(); orderedSpacingRule.layer2  = Layer::map      ( word );
...
// end TechFile private member function
